...
#from betel.models import Hospede, Familiar, Equipe, Anotacao_de_Enfermagem, Sinais_Vitais, Controle_de_Dextro, Evolucao

class AdicionaHospede(ModelForm): 
...
class AdicionaFamiliar(ModelForm): 
...
class AdicionaEquipe(ModelForm): 
...
class AdicionaFicha_de_Avaliacao(ModelForm): 
...
class AdicionaItem(ModelForm): 

    class Meta: 
        model = Item		
        fields = ['nome',]
		
class AdicionaEstoque(ModelForm): 
